...
fn run_ok(args: &[&str]) -> String {
    let (stdout, stderr, success) = run_codeview(args);
...
"#);
    let out = run_ok(&[&path, "--search", r"let \w+ = \d+"]);
...
fn search_no_matches_empty_output() {
    let out = run_ok(&["tests/fixtures/sample.rs", "--search", "zzz_nonexistent_zzz"]);
    assert!(out.trim().is_empty(), "no matches should produce empty output, got: '{}'", out);
}
...
fn search_case_insensitive() {
    ...
    let out = run_ok(&[&path, "--search", "hello"]);
    assert!(!out.contains("Hello"), "case-sensitive should not match: {}", out);

...
fn test_expand_nonexistent_symbol() {
    ...
    // Should succeed but output may be empty or minimal
    assert!(result.is_ok(), "process_path should not fail for nonexistent symbol");
    let output = result.unwrap();
    
    // Output should not contain the nonexistent symbol expanded
    // (it might show file structure but not the symbol we asked for)
    assert!(!output.contains("NonexistentSymbol {"), "Should not expand nonexistent symbol");
}
...
fn test_stats_output_plain() {
    ...
    // Stats output should contain file path and numeric info
    assert!(output.contains("files:") && output.contains("lines:") && output.contains("items:"),
            "Stats should contain summary counts. Got: {}", output);
    // Should contain lines/bytes/items counts
    assert!(output.contains("lines") || output.contains("Lines") || output.contains("items") || output.contains("Items"),
            "Stats should contain count labels. Got: {}", output);
}

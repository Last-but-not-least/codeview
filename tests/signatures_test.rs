...
fn signatures_decorated_class() {
    ...
    let output = run_codeview(&[&path, "MyWidget", "--signatures"]);
...
fn signatures_combined_expand_method() {
    ...
    let output = run_codeview(&[&path, "Greeter", "--signatures", "greet"]);
...
fn signatures_combined_multiple_methods() {
    ...
    let output = run_codeview(&[&path, "Greeter", "--signatures", "greet", "farewell"]);

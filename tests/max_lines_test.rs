...
fn run_codeview(args: &[&str]) -> String {
...
fn max_lines_truncates_long_function() {
    ...
    let output = run_codeview(&[&path, "processData", "--max-lines", "3"]);
...
fn max_lines_no_truncation_when_under_limit() {
    let dir = TempDir::new().unwrap();
    let path = write_ts_file(&dir, "test.ts", LONG_FUNCTION);
    let output = run_codeview(&[&path, "processData", "--max-lines", "500"]);
...
fn max_lines_combined_with_signatures() {
    ...
    let output = run_codeview(&[&path, "Greeter", "--signatures", "--max-lines", "2"]);
...
fn max_lines_multiple_symbols() {
    let dir = TempDir::new().unwrap();
    let path = write_ts_file(&dir, "test.ts", LONG_FUNCTION);
    let output = run_codeview(&[&path, "processData", "helperFn", "--max-lines", "2"]);
...
fn max_lines_exact_limit_no_truncation() {
    ...
    let path = write_ts_file(&dir, "test.ts", LONG_FUNCTION);
    let output = run_codeview(&[&path, "helperFn", "--max-lines", "7"]);
